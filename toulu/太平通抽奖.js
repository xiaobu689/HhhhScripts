 ...
 * const $ = new Env("太平通抽奖");
 * -----------------------------------
 * 5积分抽奖，最低得3积分，每天抽奖100次，最大损200, 运气
 * -----------------------------------
 */
...
async function main() {
    for (const [index, item] of TaiPingTong.entries()) {
        console.log("index=", index)
        if (index === 0) {
            token = item.token;
            userId = item.userId;
            let registerAndLogin = await commonPost("/tptplaybox/api/account/registerAndLogin", { "activityCode": activityCode, "phone": "", "smsCode": "", "ticket": "", "thirdAccount": token, "registerData": {} }, "", activityCode);
            let accessKey = registerAndLogin.value.accessKey;
            console.log(`用户：${userId} 开始抽奖`);
            for (let i = 0; i < 100; i++) {
                payload = {
                ...
                let businessInfo = JSON.stringify(payload);
                ...
                if (lottery.success) {
                    console.log(`第${i + 1}次 | 获得${lottery.value[0].prizeName}`);
                    notice += `第${i + 1}次 | 获得${lottery.value[0].prizeName}\n`;
                } else {
...
async function commonPost(url, body = {}, activityCode) {
