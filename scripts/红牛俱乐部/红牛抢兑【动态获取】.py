...
async def gift_list(headers):
    url = 'https://xiaodian.miyatech.com/api/coupon/integral-mall/exchange-item/query?categoryId=25&pageNo=1&pageSize=10&sortKey=&sortVal=&screenExGoods=DISABLE'

    response_json = requests.get(url, headers=headers).json()
    if response_json['code'] == '200':
        return response_json['data']['resultList']
    else:
        return None
...
async def exchange(headers, itemId, activityNo):
    body = {
    ...
    async with aiohttp.ClientSession(headers=headers) as session:
        try:
            async with session.post(url, data=body_json) as response:
                ...
                if data.get('success'):
                    message = f"✅ 兑换成功 | {data['error']} | 耗时: {duration_ms:.2f} ms | 响应时间：{end_response.strftime('%H:%M:%S.%f')[:-3]}"
                else:
                    message = f"❌ 兑换失败 | {data['error']} | 耗时：{duration_ms:.2f} ms | 响应时间：{end_response.strftime('%H:%M:%S.%f')[:-3]}"
                print(message)
                return message
...
async def main():
    ...
    token = redbull_token.get('token')
    user_id = redbull_token.get('id')
    ...
    # 调用异步函数并等待返回值
    gift_list_result = await gift_list(headers)
    if gift_list_result is not None:
        for item in gift_list_result:
            print(item)
            itemName = item.get('itemName')
            exchangeIntegral = item.get('exchangeIntegral')
            availableCount = item.get('availableCount')
            if '京东E卡10元' in itemName:
                itemId = item.get('itemId')
                activityNo = item.get('activityNo')

                # 兑换
                tasks = [exchange(redbull_token, itemId, activityNo) for _ in range(5)]
