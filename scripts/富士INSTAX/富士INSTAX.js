...
let INSTAX = ($.isNode() ? process.env.INSTAX : $.getdata('INSTAX')) || '';
...
async function main() {
    for (const _token of tokens) {
        token = _token;
        // const match = account_info.match(/Bearer\s([a-fA-F0-9]{64})#(\d+)/);
        // if (!match) {
        //     console.error(`账号信息格式不正确：${account}`);
        //     continue;
        // }
        // const user_token = `Bearer ${match[1]}`; // 包含 'Bearer ' 的 token
        // const userId = match[2]; // 提取到的 userId

        // 获取用户ID
        let user = await commonGet(`/me`);
        if (user.error == true) {
            saveResultToFile("error", name)
            $.msg("获取用户信息失败，token可能已失效");
            continue
        } else {
