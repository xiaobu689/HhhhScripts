...
class SHHP():
    ...
    # 观看直播
    def live_streaming_add(self):
        json_data = {}
        url = 'https://hpapi.shmedia.tech/media-basic-port/api/app/points/live/add'
        response_json = make_request(url, json_data=json_data, method='post', headers=self.headers)
        if response_json and response_json['code'] == 0:
            print(f'✅观看直播成功')
        else:
            print(f'❌观看直播失败，{response_json}')
